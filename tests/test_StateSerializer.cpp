...
{
    auto input = std::make_unique<PresetState>();
    input->gridData[17] = 3;
    ...
    auto output = std::make_unique<PresetState>();
...
{
    auto input = std::make_unique<ProjectState>();
    ...
    auto output = std::make_unique<ProjectState>();
...
TEST_CASE("Preset and project documents cannot be interchanged", "[state]")
{
    ...
    auto project = std::make_unique<ProjectState>();
