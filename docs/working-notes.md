...
## Current state
- ColorPalette invisibility fix (2026-08-29): v0.2.2's positioning unification introduced `m_imageVerticalShift = getY() + paletteH/2 - canvasCenterY`, which aligned the palette texture/blobs to the *canvas* center (~390px above the palette component) — JUCE clips paint to component bounds, so the entire palette (texture, 12 blobs, wheel) was invisible. Fix: removed the canvas-center-Y plumbing (`setCanvasCenterY`, `m_canvasCenterY`, `m_imageVerticalShift`) from `ColorPalette.{h,cpp}`; texture now fills its own bounds, blobs/wheel centered in the strip (`paletteH * rowRatio + contentCenterOffset`). `PluginEditor::resized()` insets the palette horizontally (`PaletteColumnInsetRatio` 0.05 → ~32px at design) so the full-bleed sprite doesn't touch window edges; `setImageCenterX` keeps palette + canvas sharing x-center 315. `CanvasScaleRatio` comment corrected (internal 90% texture scale, not a nudge). Verified: full rebuild zero warnings, 38 tests / 4.76M assertions pass.
- Cross-platform warning cleanup + version alignment (2026-08-29): clang exposes `-Wdeprecated-declarations` by default while MSVC keeps C4996 off — the Mac build showed the JUCE 8 deprecated `Font()` ctor warning that Windows never did. Fixed: `PedalComponent.h` `m_lcdFont`/`m_labelFont` now in-class-initialized via `Font(FontOptions)`; `SpriteKnob.h` ctor init list reordered to declaration order (`-Wreorder-ctor`); dead `v3Size` removed from `StateSerializer.cpp`. Version bumped 0.2.0 → 0.2.2 (`CMakeLists.txt` project + plugin, README, `docs/build.md`) to match git HEAD; CHANGELOG "Unreleased" folded into a v0.2.2 entry. Docs: `docs/build.md` verification section now documents `drawdio_tests` + `ctest` (was stale "no tests"), README gained per-platform end-user install instructions (VST3/AU paths, Gatekeeper right-click note, Windows per-user dir). Verified: full Release rebuild zero warnings (-Wall -Wextra sweep clean, 71 TUs), all 38 tests / 4.76M assertions pass.
