...
Build and install the Release VST3 bundle.
...
--parallel is not supplied. The script installs VST3 only.
EOF
}
...
BUILD_ARGS=(--build "$BUILD_DIR" --config Release --target Drawdio_VST3 --parallel)
...
install_to_directory() {
    ...
    local staging="$target_dir/.Drawdio.vst3.staging.$$"
    local backup="$target_dir/.Drawdio.vst3.previous.$$"
    ...
    if ! cp -R "$SOURCE_PLUGIN" "$staging_parent/$PLUGIN_NAME" 2>/dev/null; then
        rm -rf "$staging_parent"
        return 1
    fi
    if ! validate_bundle "$staging_parent/$PLUGIN_NAME"; then
        rm -rf "$staging_parent"
        return 1
    fi
    ...
    if ! mv "$staging_parent/$PLUGIN_NAME" "$destination" 2>/dev/null; then
        if [[ -e "$backup" ]]; then
