...
Drawdio is a JUCE 8.0.15 VST3, AU, and Standalone audio effect. It converts drawings on a 512x512 canvas into pedalboard routing, effect parameters, and DAW-synchronized automation. Six pedal slots in a 2x3 board hold 25 active DSP effects plus Bypass; allocation-free audio processing, atomic configuration handoff, and a 20 ms crossfade keep the signal path real-time safe while a background compiler turns brush strokes into sound.
...
Automation is compiled from the canvas Y-position into 128 horizontal slices (four canvas columns per slice, weighted average ignoring transparent cells). Playback follows DAW PPQ when available and falls back to 120 BPM / 0 PPQ.
...
- **Save / Import** — presets write a versioned JUCE `ValueTree` (SchemaVersion 4, `DrawdioState` root, `type`/`version` properties) to `.drawdio` files containing canvas data, pedal slots, routing, knob values, override mask, link flags/ranges, bar/section settings, mode, gains, and the 128-slice manual envelope. Importing a `.drawdio` replaces the preset layer and preserves the current editor session. Preset save/import share one split pill (`[ SAVE | IMPORT ]`), and canvas image import/export share a second (`[ IMPORT | EXPORT ]`, 512x512 transparent PNG).
...
For a Release VST3 build and installation, use `./updater.sh` on macOS, Linux, or Windows Git Bash, `updater.ps1` from PowerShell, or `updater.cmd` from Command Prompt. The updater builds VST3 + Standalone (+ AU on macOS) and installs the VST3; Standalone/AU artefacts are validated. See [`docs/build.md`](docs/build.md#release-updater) for installation paths and permission behavior.
...
- [`docs/state-format.md`](docs/state-format.md) - preset serialization (SchemaVersion 4)
