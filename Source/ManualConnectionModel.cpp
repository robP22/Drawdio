...
void ManualConnectionModel::removeEdgesWithSource(uint8_t src)
{
    m_edges.erase(std::remove_if(m_edges.begin(), m_edges.end(),
        [src](const auto& e) { return e.first == src; }), m_edges.end());
}

void ManualConnectionModel::removeEdgesWithDestination(uint8_t dst)
{
    m_edges.erase(std::remove_if(m_edges.begin(), m_edges.end(),
        [dst](const auto& e) { return e.second == dst; }), m_edges.end());
}

void ManualConnectionModel::removeAllEdgesForSlot(uint8_t slot)
{
    m_edges.erase(std::remove_if(m_edges.begin(), m_edges.end(),
        [slot](const auto& e) { return e.first == slot || e.second == slot; }), m_edges.end());
}

void ManualConnectionModel::removeConflictingEdges(uint8_t src, uint8_t dst)
{
    m_edges.erase(std::remove_if(m_edges.begin(), m_edges.end(),
        [src, dst](const auto& e) { return e.first == src || e.second == dst; }), m_edges.end());
}
...
void ManualConnectionModel::commitTo(CanvasRoutingManager& rm, DrawdioProcessor& proc) const
{
    auto order = deriveRoutingOrder();
    if (order != rm.getRoutingOrder())
    {
        rm.setManualRoutingOrder(order);
        proc.setManualRouting(order);
    }
}
