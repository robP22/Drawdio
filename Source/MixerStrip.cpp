...
MixerStrip::MixerStrip(DrawdioProcessor& proc, int slotIdx)
    : m_processor(proc), m_slotIndex(slotIdx)
{
    startTimerHz(20);
}
...
void MixerStrip::paint(juce::Graphics& g)
{
    ...
    bool active = (m_processor.getPedalSlot(m_slotIndex) != DspModuleType::BYPASS);
    ...
    if (active)
    {
        float fillH = m_meterBounds.getHeight() * juce::jlimit(0.0f, 1.0f, juce::jmap(juce::Decibels::gainToDecibels(m_displayPeak), -32.0f, 6.0f, 0.0f, 1.0f));
    ...
    if (active)
    {
        ...
        float thumbY = m_sliderBounds.getY() + m_sliderBounds.getHeight() * (1.0f - juce::jlimit(0.0f, 1.0f, juce::jmap(juce::Decibels::gainToDecibels(m_displayGain), -32.0f, 6.0f, 0.0f, 1.0f)));
...
{
    if (m_processor.getPedalSlot(m_slotIndex) == DspModuleType::BYPASS) return;
    const float h = getHeight();
    ...
    if (m_sliderBounds.expanded(expand).contains(e.position))
    {
        ...
        m_processor.getDSPProcessor().setPedalGain(m_slotIndex, m_displayGain);
        repaint();
    }
}
...
{
    float peak = m_processor.getDSPProcessor().getPedalPeak(m_slotIndex);
    float gain = m_processor.getDSPProcessor().getPedalGain(m_slotIndex);
