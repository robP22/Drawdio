...
void FlangerEffect::prepare(double sampleRate, int numChannels)
{
    ...
    m_minDelaySamples = static_cast<float>(sampleRate) * 0.0005f;
...
{
    juce::ScopedNoDenormals noDenorm;
    const float rateHz = 0.1f + params[1] * 4.9f;
    const float depth = std::max(0.0f, std::min(1.0f, params[2]));
    const float feedback = std::max(0.0f, std::min(0.9f, params[3]));
    const float sr = static_cast<float>(m_sampleRate);
    const float inc = rateHz / sr;
    ...
    for (int ch = 0; ch < chCount; ++ch)
    {
        ...
        for (int s = 0; s < n; ++s)
        {
            ...
            mc.buf[mc.writePtr] = in + std::tanh(delayed * feedback);
            b[ch][s] = delayed;
